# scratch_phase2_check.jl
#
# Standalone Phase 2 sanity check (NOT part of test/ yet).
#
# Test curve: the nodal cubic f(x,y) = y^2 - x^3 - x^2, which has a
# genuine node (self-intersection) at the origin -- both ∂f/∂x and
# ∂f/∂y vanish there -- and is otherwise smooth. This lets us exercise
# both the `Critical` and `Singular` classification paths from a single
# small, hand-verifiable curve (worked out by hand below).
#
# Run with:
#   julia --project=. scratch_phase2_check.jl
...
println()
println("All Phase 2 sanity checks PASSED.")
