# scratch_phase4_check.jl
#
# Standalone Phase 4 sanity check (NOT part of test/ yet), continuing
# directly from Phase 2/3's nodal cubic. Phase 4 is a REFACTOR of
# Phase 3's already-tested tracking behavior (see scratch_phase3_check.jl,
# which must still pass unchanged -- that script IS this refactor's
# regression check), plus new capability (public API, generalized
# vector-valued state, cfg.path_tracker_precision wiring). This script
# specifically exercises the new src/PathTracking.jl public API
# (`is_near_singular`, `build_tracker`, `track_path`,
# `track_bidirectional`) directly, bypassing `connect_the_dots!`/
# `decompose_1d_curve` entirely, to confirm the engine is independently
# testable on its own.
#
# Run with:
#   julia --project=. scratch_phase4_check.jl
...
println()
println("=" ^ 70)
println("All Phase 4 sanity checks PASSED.")
println("=" ^ 70)
