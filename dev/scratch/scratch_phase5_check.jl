...
#
# Standalone Phase 5 sanity check (NOT part of test/ yet), continuing
# directly from Phase 2 (Solver.jl)/Phase 3 (Topology.jl)/Phase 4
# (PathTracking.jl). Phase 5 introduces genuinely NEW machinery
# (src/FaceTracking.jl, src/SurfaceDecomposition.jl) rather than
# refactoring existing behavior, so this script exercises it against a
# fully hand-verifiable test surface: the UNIT SPHERE
#   f(x,y,z) = x^2 + y^2 + z^2 - 1
# chosen (per the approved architecture prompt) as simpler than the
# Taubin heart for a first pass, while still exercising every piece of
# the pipeline non-trivially:
#   - genuine critical z-slices at z = -1, +1 (the poles, where
#     ∂f/∂x = ∂f/∂y = 0)
#   - a smooth, hand-computable mid-slice at z = 0 (the unit circle,
#     with x-projection critical points at exactly (±1, 0))
#   - swept surface points that must satisfy x^2+y^2+z^2 = 1 EXACTLY
#     (a strong, independent correctness check on every tracked point,
#     not just endpoints)
#   - an unambiguous outward-normal convention (∇f = (2x,2y,2z) points
#     radially outward everywhere on the sphere, so `weld_mesh`'s
#     winding-fix can be checked against "normal · position > 0" at
#     every single triangle, not just spot-checked).
#
# Run with:
#   julia --project=. scratch_phase5_check.jl
...
println()
println("=" ^ 70)
println("All Phase 5 sanity checks PASSED.")
println("=" ^ 70)
