...
@with_kw struct HomotopyConfig{T<:AbstractFloat}
    ...
    # projection_orthonormality_tol: acceptance threshold for a user-supplied
    # projection matrix's orthonormality defect, norm(Q'Q - I), inside
    # Projection._resolve_projection. Deliberately its own field, not a reuse
    # of jacobian_rank_tol (same default magnitude today, but a different
    # physical quantity -- a matrix-identity defect, not a Jacobian
    # singular-value cutoff); see this file's own header comment on why
    # mixing differently-scoped tolerances is a known bug source here. See
    # docs/DESIGN_NOTES.md, "Generic projection support (Phase 8)", for why
    # this field exists rather than a bare literal.
